...
pub fn compile_to_net(term: &CoreIR) -> Result<Net, String> {
    let mut net = Net::new();
...
fn emit(term: &CoreIR, net: &mut Net) -> Result<Port, String> {
    match term {
        CoreIR::Var(id) => {
...
mod tests {
    ...
    fn test_emit_literal() {
        ...
        let net = compile_to_net(&ir).unwrap();
    ...
    fn test_emit_app() {
        ...
        let root = emit(&ir, &mut net).unwrap();
        assert!(matches!(
