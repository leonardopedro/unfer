...
use nested_fock_algebra::{
    ...
    inner_boson_annihilate, inner_boson_create,
};
...
fn main() -> anyhow::Result<()> {
    ...
    for i in 0..4 {
        h_expr = h_expr
            + inner_boson_create(i + 1) * inner_boson_annihilate(i)
    ...
    let initial_state = QuantumState::vacuum().apply(&Operator::OuterBosonCreate(inner_b));
