...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Hamiltonian has {} operator terms.",
        hamiltonian.terms.len()
    );

    // Initial state: Vacuum (no gluons)
    let mut v_0 = QuantumState::vacuum();
    v_0 = v_0.apply(&nested_fock_algebra::Operator::OuterBosonCreate(
    ...
    println!(
        "Subspace built. G-matrix shape: {}x{}",
        sirk_result.g_matrix.nrows(),
        sirk_result.g_matrix.ncols()
    );

    let t = 0.1;
    let coeffs = sirk_result.time_evolve(t);
    println!(
        "Gluon vacuum fluctuation coefficients at t={}: {:?}",
        t, coeffs
    );
