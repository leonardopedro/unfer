...
impl ForwardSirkResult {
    ...
    ///
    /// **Sector caveat:** a single Krylov solve sees only the invariant sector
    /// generated by its start. For the gauge-fixed nested-Fock QYM Hamiltonian,
    /// use the exact reflection sectors, not occupation parity. The physical
    /// Hamiltonian is the complete one-particle `h` enclosed by outer creation
    /// on the left and annihilation on the right; its outer vacuum is therefore
    /// the exact ground after the allowed positivity shift. Sector solves are
    /// one-particle spectral diagnostics used to estimate the edge above it.
...
/// parity sectors.
///
/// For the gauge-fixed nested-Fock QYM Hamiltonian, this function compares
/// independent reflection-sector SIRK–Hashimoto solves. The sector values are
/// diagnostics of the complete inner one-particle Hamiltonian; the full
/// physical operator is its outer creation-left/annihilation-right enclosure,
/// whose outer vacuum is the exact ground after the allowed positivity shift.
/// Historical lattice parity and `g²/2` statements are comparison-only.
///
...
/// Hamiltonian; historical occupation-parity/lattice models are comparison
/// models only), enforces the theorem's checkable precondition at runtime
...
///    (`< 1e-8`).
///
/// The inner normal-ordering reference `⟨0|h|0⟩ = 0` does not identify the
/// inner spectral minimizer when pair terms are present. Those squeezed inner
/// levels are not the full-theory ground: the outer enclosure makes the outer
/// vacuum exact. The certified statement here is the *enclosure* of the
/// one-particle sector edge by
