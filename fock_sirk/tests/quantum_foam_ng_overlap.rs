...
use nested_fock_algebra::{
    InnerBosonicState, Operator, QuantumState, qg_free_graviton,
};
...
fn qf_ng_graviton_zero_point_energy() {
    let l = 1.0_f64;
    let omegas: Vec<f64> = (1..=4).map(|n| n as f64 * std::f64::consts::PI / l).collect();
...
fn qf_ng_graviton_number_conservation() {
    ...
    let h = qg_free_graviton(&[omega]);
    for n in 0..=4 {
        let res = solve_forward_sirk_with_opts(&h, &n_photon(0u32, n as u32), &shifts(6),
            &best_device(), None, &opts()).expect("SIRK");
        let ritz = res.ritz_values();
        let has = ritz.iter().any(|&r| (r - n as f64 * omega).abs() < 0.1);
        assert!(has, "n={n}: Ritz {ritz:?} must contain nω={:.1}", n as f64 * omega);
    }
