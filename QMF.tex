...
\date{June 2026 (rev.\ 4, with Tomographic Subspace Recovery, rev 14 SIRK-backed unitary flow, rev 15 module demo + benchmarks, rev 16 Quantum Bayesian Update implementation)}
...
\subsection{Implementation status (rev 16, 2026-06-30)}

The Bayesian-update algorithm of \S\ref{sec:bayes} is implemented
in the \texttt{unfer} kernel as a new \texttt{qfm::bayes} module
(rev 16, \texttt{commit 47f6a0c} + \texttt{5e0b87b}). The module
exports:
...
\item \texttt{sample\_hmc(posterior, opts)} and
  ...
  constrained to the natural manifold of the wavefunctions). Uses
  a deterministic splitmix64 + Box--Muller PRNG (no external
  dependencies).
...
The implementation has seven new unit tests
(\texttt{+7, 163} $\to$ \texttt{170} workspace), including the
acceptance criterion \texttt{bayesian\_update\_tsr\_recovers\_training\_mode}:
on the 4-pt tetrahedron training set with a single observation
at training point $\vec e_0$, the HMC sample decodes to an image
whose $\arg\max$ is at index 0 --- exactly the training point. A
fourth benchmark group \texttt{bayes\_update\_vs\_n}
($N = 1, 4, 16$) confirms the $\mathcal{O}(N \cdot m^2)$ per-step
HMC cost. Not yet wired into \texttt{prob\_kernel::Session} or the
\texttt{uk\_*} FFI surface (~1 day of follow-on work).
