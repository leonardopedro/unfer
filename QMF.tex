...
\textbf{16 of 18}.
The remaining 2 are research-grade (P10.16 real-dataset
scaling, P10.18 Millennium-Prize mass-gap demo) and are
documented as the next steps below. P10.17 (Lindbladian)
is explicitly out of scope per user decision (2026-06-30).
...
\subsubsection*{Next steps to improve (post-rev 23)}

The remaining P7--P10 items (2 of 18) are the documented next
steps:

\begin{enumerate}
\item \textbf{P9.15.1 port the deleted velyst examples to the
  velyst 0.15 API} (DONE in rev 23, $\sim$1--2 days). The
  velysterm-vendored \texttt{editor.rs} / \texttt{terminal.rs} /
  \texttt{rfc1751\_demo.rs} are re-introduced in
  \texttt{velysterm/examples/velyst\_demo/examples/} and ported
  to the velyst 0.15 + typst 0.15 + Bevy 0.18 API surface. (Now
  kept here for the historical record; the implementation lives
  in the velysterm repo, not the unfer repo.)
