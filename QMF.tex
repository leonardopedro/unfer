...
The remaining 4 are research-grade (P10.16 real-dataset
scaling, P10.18 Millennium-Prize mass-gap demo) or cross-repo
(P9.14 \texttt{mathed\_mini} Step 4, P9.15.1 port the deleted
velyst examples to velyst 0.15) and are documented as the
next steps below. P10.17 (Lindbladian) is explicitly
out of scope per user decision (2026-06-30).
...
\subsubsection*{Next steps to improve (post-rev 21)}

The remaining P7--P10 items (4 of 18) are the documented next
steps:
...
\item \textbf{P9.15.1 port the deleted velyst examples to the
  velyst 0.15 API} (deferred follow-on, $\sim$1--2 days). The
  velysterm-vendored \texttt{editor.rs} / \texttt{terminal.rs} /
  \texttt{rfc1751\_demo.rs} were triply-stale against the
  velyst 0.15 + \texttt{typst 0.15} API surface and were
  \textbf{deleted} in the rev 21 velysterm merge. Porting them
  to the new API
  (\texttt{VelystSourceHandle} $\to$ \texttt{VelystSource},
  \texttt{VelystFuncBundle} $\to$ \texttt{VelystContent}, the
  new \texttt{register\_typst\_func::<T>()} macro surface, the
  new \texttt{Engine} struct, the new \texttt{PaintSink} trait
  from \texttt{typst\_imaging} 0.15) is a non-trivial rewrite
  that is deferred as a follow-on revision.

\item \textbf{P10.16 real-dataset QFM-TSR validation} (ongoing
  research; \textbf{recommended next revision}). Extend the
  P7.3 MNIST 8$\times$8 baseline to (a) larger image datasets
  (CIFAR-10 32$\times$32, ImageNet 64$\times$64); (b)
  cross-domain datasets (CFD flow fields, 2D Ising-model
  critical snapshots, gravitational waveforms); (c) the
  Bayesian-update posterior quality on adversarial / partial
  observations. This is the central scientific claim of the
  QFM-TSR pipeline and has no size cap.
...
\subsubsection*{Items DONE in rev 20--21 (now in the past, kept here for the historical record)}
