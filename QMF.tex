...
\textbf{Test counts (CPU, full sweep 2026-06-30, rev 18/24):}
unfer workspace \textbf{227} as of rev 24 (was 196 at rev 18; +31 over P8--P10 revisions; breakdown: 19 fock\_sirk + 26
nested\_fock\_algebra + 37 prob\_kernel + 39 unfer\_protocol +
33 unfer\_ffi + 39 qfm + 4 mnist\_validation + 4 cifar10\_validation); 6 module demos
...
\textbf{16 of 18}.
The remaining items are research-grade: P10.16.2+ real-dataset
scaling sub-revisions (P10.16.1 DONE in rev 24 --- see below)
and P10.18 Millennium-Prize mass-gap demo; both are documented
as the next steps below. P10.17 (Lindbladian)
...
$>0.0$); (3) the QMF.tex \S8 Bayesian update on held-out images
yields $\max|\langle\text{sample}\mid c_{\text{obs}}\rangle|^2
> 0.01$; (4) no NaN/Inf in held-out \texttt{generate}.  The
\texttt{build\_flow\_hamiltonian} ``star'' structure (vacuum
coupled to $K_2$ modes with coupling $\alpha_j/2 = \|x_j\|^2/(2M)$)
produces an effective 2D Krylov subspace after whitening; the
$\sim$2--9 whitened rank is the documented behaviour for
small-rank flow Hamiltonians.

\subsubsection*{Next steps to improve (post-rev 24)}

The remaining P7--P10 items (1.5 of 18, since P10.16 is ongoing)
are:

\begin{enumerate}
\item \textbf{P10.16.1} (DONE in rev 24 --- see above).

\item \textbf{P10.16.2+ real-dataset QFM-TSR validation}
  (ongoing research). Sub-steps: (a) replace the synthetic
  CIFAR-10 fixture with a real one (torchvision script
  $\to$ JSON, same format as \texttt{mnist\_8x8\_65training.json});
  (b) implement the rank-truncation extension point
  (\S11) to support $d=1024$ (CIFAR-10 32$\times$32) without
  the $O(K_2^3)$ W\_prob bottleneck; (c) cross-domain
  datasets (CFD flow fields, 2D Ising-model critical snapshots,
  gravitational waveforms).  Each sub-revision is independent.
