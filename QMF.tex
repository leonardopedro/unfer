...
\date{June 2026 (rev.\ 5, with Tomographic Subspace Recovery, rev 14 SIRK-backed unitary flow, rev 15 module demo + benchmarks, rev 16 Quantum Bayesian Update implementation, rev 17 full SIRK-whitened Krylov basis + kernel surface wiring + 5th module demo, rev 18 productionization: P7 (README, CUDA-on-CI, MNIST 8x8 real-data validation, runtime krylov_dim$\ge$K$_2$ check, HMC validation, 3 new bench groups) + P8.10 (CUDA kernel bench) + P8.11 (iterated\_bayes\_module 6th demo) + P9.12 (CUDA toolkit pinning) + P9.13 (CI PAT docs))}
...
\subsection{Current state and next steps (rev 18, 2026-06-30)}
...
\S\ref{sec:impl} is the \texttt{qfm\_mehler} builtin; the
off-diagonal $\Hgen_{\mathrm{off}}$ coupling is the
...
\textbf{Test counts (CPU, full sweep 2026-06-30, rev 18):}
unfer workspace \textbf{196} (19 fock\_sirk + 26
nested\_fock\_algebra + 37 prob\_kernel + 39 unfer\_protocol +
33 unfer\_ffi + 39 qfm + 4 mnist\_validation); 6 module demos
(positive + UK-4001 negative); 4 criterion benchmark groups in
...
The remaining 8 are research-grade (P8.7 Typst-math compiler,
P8.8 belief propagation, P10.16 real-dataset scaling, P10.17
Lindbladian, P10.18 Millennium-Prize mass-gap demo) or
cross-repo (P9.14 \texttt{mathed\_mini} Step 4, P9.15 velyst
examples) and are documented as the next steps below.
...
\item \textbf{README refresh (P7.1).} The pre-modular-kernel
  ...
  links to \texttt{QMF.tex}, \texttt{IMPLEMENTATION\_PLAN.md},
...
\paragraph{P8 user-facing (2 of 5 items done).}
...
\item \textbf{\texttt{iterated\_bayes\_module} (P8.11).} New 6th
  ...
  drives the full QMF.tex \S7 + \S8 pipeline (TSR + Bayesian
...
\subsubsection*{Next steps to improve (post-rev 18)}

The remaining P7--P10 items (8 of 18) are the documented next
steps:

\begin{enumerate}
\item \textbf{P8.7 Typst-math $\to$ Hamiltonian compiler}
  ($\sim$2 weeks, research). The v1 translator pipeline has the
  user author a Typst translator function that maps rendered math
  $\to$ \texttt{TermSpec[]}. A general compiler that maps
  rendered math \emph{directly} to a Hamiltonian (no
  hand-authored translator) remains the v1 documented extension
  point. The compiler can use the existing
  \texttt{compile\_latex} as a backend; the main work is the
  Typst math AST $\to$ LaTeX or \texttt{TermSpec} mapping.
  Add \texttt{compile\_typst\_math(input: \&str) $\to$ Hamiltonian}
  to \texttt{nested\_fock\_algebra}, wire into
  \texttt{prob\_kernel::HamiltonianSpec::Typst}, and add
  \texttt{+5} tests.

\item \textbf{P8.8 \texttt{uk\_belief\_propagation} FFI symbol}
  ($\sim$1 week, research). A v2 message-passing alternative to
  HMC for product-of-likelihoods posteriors. The QFM posterior
  is a product of rank-1 likelihoods; BP is exact for
  tree-structured observation graphs and a fast approximation
  otherwise. Adds a kernel symbol
  \texttt{uk\_belief\_propagation(model, obs\_graph\_json, len)
  $\to$ PosteriorSampleReport} that returns the BP-estimated
  marginal mean of the latent. New
  \texttt{qfm::bayes::belief\_propagation(pipeline, graph, opts)}
  module function.

\item \textbf{P8.9 \texttt{bayes\_update\_module} v2} ($\sim$1
  week, research). The current \texttt{bayes\_update\_module/}
  hardcodes a single observation. A v2 demo could drive
  multiple observations in a loop (consume the JSON result,
  feed it back as a new observation), demonstrate a
  posterior-mean estimate (sample $N$ points, compute the
  Karcher mean on the unit sphere of $\Cset^m$), and expose a
  \texttt{generate\_from\_observations} op that returns the full
  \texttt{BayesianUpdateResult} (not just the HMC sample).

\item \textbf{P9.14 velysterm \texttt{mathed\_mini} Step 4}
  ($\sim$1 week, cross-repo). Caret blink via
  \texttt{ControlFlow::WaitUntil}; mouse hit-testing /
  click-to-place-caret + selection (the \texttt{byte\_for\_point} /
  \texttt{rects\_for\_range} plumbing already exists);
  \texttt{mathed\_a11y} AccessKit bridge over
  \texttt{mathed\_core::accessibility}; and the headline item:
  wire \texttt{kernel\_client} into \texttt{mathed\_mini} so the
  Bevy-free frontend can show \texttt{\textbackslash prob} results
  too (today only the Bevy \texttt{mathed} frontend has the
  kernel bridge --- P3.10+11 closed that for the Bevy bridge,
  but the Bevy-free bridge is still missing).

\item \textbf{P9.15 port the stale \texttt{velyst} examples}
  ($\sim$1 day). The two upstream-vendored \texttt{velyst}
  examples (\texttt{editor}, \texttt{terminal}, referencing the
  removed \texttt{VelystFuncBundle} / \texttt{VelystSourceHandle}
  APIs) are gated behind a non-default
  \texttt{upstream-stale-examples} feature. Port them to the
  current \texttt{velyst} API and retire the feature gate.

\item \textbf{P10.16 real-dataset QFM-TSR validation} (ongoing
  research). Extend the P7.3 MNIST 8$\times$8 baseline to (a)
  larger image datasets (CIFAR-10 32$\times$32, ImageNet
  64$\times$64); (b) cross-domain datasets (CFD flow fields,
  2D Ising-model critical snapshots, gravitational waveforms);
  (c) the Bayesian-update posterior quality on adversarial /
  partial observations.

\item \textbf{P10.17 open-system / Lindbladian dynamics solver}
  ($\sim$1 month, research). The QFM Fokker--Planck transport of
  \S\ref{sec:framework} is currently approximated as a coherent
  Rabi oscillation. A full Lindbladian
  $e^{(H - i \Sigma L_k^\dagger L_k / 2)\,t}$ solver is the
  natural next step for a true generative flow that mixes modes,
  not just oscillates. The Hermitian off-diagonal QFM coupling
  (\texttt{qfm\_hamiltonian\_offdiag}) already exists as the seed.

\item \textbf{P10.18 Millennium-Prize mass-gap demonstration}
  ($\sim$2--3 months, research). The unfer architecture has all
  the pieces: \texttt{ForwardSirkResult::mass\_gap\_from\_sectors}
  (P6 A1) gives the cross-sector gap; the QFM-TSR pipeline
  (Workstream F) gives the offline training; the TSR-evolved
  prior (P6 H) gives the informed posterior. A complete
  demonstration would compile the 4D SU(2) Yang-Mills Hamiltonian
  on an $l=3$ lattice (8 plaquettes, $\sim 200$ modes), train
  the QFM-TSR pipeline on the lattice ground states, compute
  the mass gap via \texttt{mass\_gap\_from\_sectors}, and report.
  The \texttt{yang\_mills\_lattice\_mass\_gap} test (P6 A1) verified
  this at $l=2, g=2$; the demonstration would extend to $l=3$ or
  $l=4$.
\end{enumerate}
