...
fn run(
    ...
    let pipe = QfmPipeline::compile_channels_with_kernel(
        &im,
        &om,
        &tr,
        l0,
    ...
    let classify = |img_idx: usize, true_label: u32| -> bool {
        ...
        for f in 0..no {
            let tp = img_idx * no + f;
            for r in 0..rk {
                c0[r] += w[(tp, r)] * C64::new(1.0 / (no as f64).sqrt(), 0.0);
            }
        }
        let c1 = &u * c0;
        ...
        for f in 0..no {
            let tp = img_idx * no + f;
            let a: C64 = (0..rk).map(|r| c1[r] * w[(tp, r)].conj()).sum();
            pr[f] = a.norm_sqr();
        }
    ...
    let mut tc = 0u32;
    for ti in 0..nt {
        if classify(ti, train[ti].0) {
            tc += 1;
        }
    }
    let mut xc = 0u32;
    for ti in 0..nx {
        if classify(nt + ti, test[ti].0) {
            xc += 1;
        }
    }
    let htag = match htype {
        HamiltonianType::Diffusion => "diff",
        HamiltonianType::PauliGrover => "pg",
    };
    eprintln!(
        "    {htag} σ={sigma:.1} m={m:>3} λ₀={l0:.1} λ₁={l1:.1} t={time:.2} rank={rk}  train {tc}/{nt} ({:.1}%)  test {xc}/{nx} ({:.1}%)",
        tc as f64 / nt as f64 * 100.0,
        xc as f64 / nx as f64 * 100.0
    );
}
...
fn mnist_classification_tests() {
    ...
    let train: Vec<(u32, Vec<f64>)> = train_idx
        .iter()
        .map(|&i| (samples[i].0, samples[i].1.clone()))
        .collect();
    let test: Vec<(u32, Vec<f64>)> = test_idx
        .iter()
        .map(|&i| (samples[i].0, samples[i].1.clone()))
        .collect();
    eprintln!(
        "=== MNIST 8x8 (resplit: {}/{} train/test) ===",
        train.len(),
        test.len()
    );
    ...
    for &m in &[5, 10, 15, 20] {
        ...
        let pipe = QfmPipeline::compile_channels(
            &im, &om, &tr, 1.0, 1.0, nm, &cfg, 0.0, 0.0, true, None, None,
        )
        .unwrap();
        let rk = pipe.rank();
        let w = pipe.w();
        let h = pipe.h_m();
        let u = (h.clone() * (-C64::new(0.0, 1.0) * 1.0)).exp();
        let no = 5;
        ...
        for ti in 0..test.len() {
            if classify(train.len() + ti, test[ti].0) {
                xc += 1;
            }
        }
        eprintln!(
            "    vacuum start  m={m:>3} rank={rk}  train {tc}/{} ({:.1}%)  test {xc}/{} ({:.1}%)",
            train.len(),
            tc as f64 / train.len() as f64 * 100.0,
            test.len(),
            xc as f64 / test.len() as f64 * 100.0
        );
    }
    ...
    for &m in &[5, 10, 15, 20] {
        ...
        eprintln!(
            "    random start m={m:>3} rank={rk}  train {tc}/{} ({:.1}%)  test {xc}/{} ({:.1}%)",
            train.len(),
            tc as f64 / train.len() as f64 * 100.0,
            test.len(),
            xc as f64 / test.len() as f64 * 100.0
    ...
    for &m in &[5, 10, 15, 20] {
        ...
        let pipe = QfmPipeline::compile_channels(
            &im, &om, &tr, 0.5, 0.5, nm, &cfg, 0.0, 0.0, true, None, None,
        )
        .unwrap();
        let rk = pipe.rank();
        let w = pipe.w();
        ...
        eprintln!(
            "    λ/2 vacuum   m={m:>3} rank={rk}  train {tc}/{} ({:.1}%)  test {xc}/{} ({:.1}%)",
            train.len(),
            tc as f64 / train.len() as f64 * 100.0,
            test.len(),
            xc as f64 / test.len() as f64 * 100.0
    ...
    for &m in &[5, 10, 15, 20] {
        ...
        eprintln!(
            "    λ/2 random  m={m:>3} rank={rk}  train {tc}/{} ({:.1}%)  test {xc}/{} ({:.1}%)",
            train.len(),
            tc as f64 / train.len() as f64 * 100.0,
            test.len(),
            xc as f64 / test.len() as f64 * 100.0
    ...
    for &rst in &[false, true] {
        ...
        for &m in &[5, 10, 15, 20] {
            let cfg = QfmConfig {
                ...
                max_rank: None,
                random_start: rst,
                hamiltonian_type: HamiltonianType::PauliGrover,
                pauli_grover_a: 0.9,
            };
            let pipe = QfmPipeline::compile_channels(
                &im, &om, &tr, 0.0, 0.0, nm, &cfg, 0.0, 0.0, true, None, None,
            )
            .unwrap();
            let rk = pipe.rank();
            let w = pipe.w();
            ...
            eprintln!(
                "    pg a=0.9 {rtag:>7} m={m:>3} rank={rk}  train {tc}/{} ({:.1}%)  test {xc}/{} ({:.1}%)",
                train.len(),
                tc as f64 / train.len() as f64 * 100.0,
                test.len(),
                xc as f64 / test.len() as f64 * 100.0
