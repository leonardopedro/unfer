...
fn run_test(
    ...
    eprintln!(
        "  {label:>24}  m={m} rank={:2} λ₀={lambda0:>7} λ₁={lambda1:>5}  train {tc:>3}/{tn} ({pct_t:>5.1}%)  interp {ic:>2}/{it} ({pct_i:>5.1}%)  extrap {ec:>2}/{et} ({pct_e:>5.1}%)",
        pipeline.rank()
    );
}
...
fn even_odd_split(n_even: usize, n_odd: usize, pool: &[u32]) -> Vec<u32> {
    ...
    for _ in 0..n_even.min(evens.len()) {
        selected.push(evens.remove(0));
    }
    for _ in 0..n_odd.min(odds.len()) {
        selected.push(odds.remove(0));
    }
    selected.sort();
    selected
}
...
fn large_parity_classification_tests() {
    ...
    let interp_inputs: Vec<u32> = pool
        .iter()
        .filter(|x| !train_inputs.contains(x))
        .copied()
        .collect();
    ...
    eprintln!(
        "     interp: {} (within-pool held-out)",
        interp_inputs.len()
    );
    eprintln!(
        "     extrap: {} (beyond training range)",
        extrap_inputs.len()
    );
    ...
    for &rst in &[false, true] {
        ...
        for &m in &[1, 2, 3, 4, 6, 8, 10] {
            ...
            let eval = |x: u32| -> bool {
                ...
                let c1 = &u * c0;
                let a_even: Complex64 = (0..rank)
                    .map(|k| c1[k] * w[(x as usize * n_out, k)].conj())
                    .sum();
                let a_odd: Complex64 = (0..rank)
                    .map(|k| c1[k] * w[(x as usize * n_out + 1, k)].conj())
                    .sum();
            ...
            let (ec, et) = count(&extrap_inputs);
            eprintln!(
                "  PG a=0.9 {rtag:>7}  m={m} rank={rank}  train {tc:>3}/{tn} ({:.1}%)  interp {ic:>2}/{it} ({:.1}%)  extrap {ec:>2}/{et} ({:.1}%)",
                tc as f64 / tn as f64 * 100.0,
                ic as f64 / it as f64 * 100.0,
                ec as f64 / et as f64 * 100.0
            );
        }
    }
}
