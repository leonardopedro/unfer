...
two further contributions. First, the Hermitian off-diagonal vacuum--data coupling
...
wavefunction flow --- is realized in Hermitian (symmetrized) form by the
companion builtin of Section~\ref{sec:impl}.
...
The diagonal and Hermitian off-diagonal generators both encode each of the
...
\textbf{Test counts (CPU, full sweep 2026-07-02, rev 28):}
unfer workspace \textbf{251} passing under \texttt{cargo test --workspace}
(was 196 at rev 18, 227 at rev 24, 240 at rev 27 under the historical
...
\subsubsection*{Next steps to improve (post-rev 29)}

All 18 P7--P10 items and all 5 P11 integrations are closed; what remains
is open-ended follow-on work, not a scoped checklist:

\begin{enumerate}
\item \textbf{Higher-resolution / richer cross-domain datasets (P10.16.7+).}
  Scale the CFD / gravitational-waveform / Ising fixtures to 32$\times$32
  (= $d=1024$) using the P10.16.3 rank-truncation path; this is the
  natural next step after the rev 29 16$\times$16 cross-domain baselines.
