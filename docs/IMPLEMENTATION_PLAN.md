...
9. **CPS-JIT backend: record/cross-module support — LARGELY RESOLVED (2026-06-26).** The experimental safestos CPS-JIT originally executed only `uk_*` foreign calls. Found and fixed four backend bugs so the linear `Model` wrapper now **runs end-to-end through the JIT** (`DemoModule` calls `kernelVersion`, then cross-module `wrapModel`→`freeModel`, the latter destructuring the linear record to recover the handle; `run_demo.sh` shows `Execution result: 1`):
   ...
   **Update (2026-06-26) — byte buffers RESOLVED; a module now computes a real probability.** Two more fixes landed:
   ...
   - **Multi-field record sizing**: `MRecordConstructor` now sizes `__record_new` from its own field count instead of the (unregistered, for concrete records) layout, so multi-field records no longer under-allocate / corrupt the heap.

   **Remaining (minor):** concrete (non-generic) multi-field records still aren't in `record_layouts` (keyed by `mono_id` from `MonoNamedType`, while `MRecord` carries a `decl_id`), so **slot *offsets*** for fields beyond the first default to 0 — multi-field *destructure/access* reads the wrong field. Construction is safe; single-field records (`Model`, `Span`) are fully correct. Reconciling the `decl_id`↔`mono_id` mapping is the last open item.
