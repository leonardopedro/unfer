...
- **Git state (all three repos pushed to their remotes):**
  ...
  - velysterm HEAD `0916146` → `gitbutler/workspace` (non-default branch, push not harness-blocked). The translator pipeline (P3 #10 Steps 1–6 + P3 #11) **and** the follow-ups are committed. Follow-ups (`0916146`): translator-aware caching (hashes fold in the *resolved* translator source via `dispatch::resolve_translator_src`, so editing even an unnamed `""` default re-dispatches dependents), multi-model `model: "name"` binding, richer event predicates + typed `EventPredicate` validation, `builtin_event_translator.typ`, a clippy fix in `doc.rs`. All tests green (mathed_core 71, mathed_mini 31), clippy clean on affected crates. **Needs push** (`git push origin gitbutler/workspace`).
- **Progress checklist:**
...
15. **Wire `\prior` and `\solver` segments through the dispatcher.** Today `PropKind::Prior` and `\prior(#1,#2)` segments are collected into `SemanticIndex.kernel_statements` (S15) but **ignored** — `dispatch::statement_to_model_spec` hardcodes `prior: PriorSpec::Vacuum` and `solver: SolverSpec::default()` (`dispatch.rs:100-101`). The `\solver` PropKind doesn't exist yet. This is the single biggest gap in the document-driven model spec: a user can define a `\model` and `\prob` but cannot set a non-vacuum prior or tune the solver from the document.
    - Add `PropKind::Solver` to `mathed_core/src/markers.rs` (`of("solver")`, `is_kernel()` → true).
    - Extend `KernelStatement` (already has `kind`/`body_text`/`span`) — the dispatcher needs to associate `\prior`/`\solver` with their nearest preceding `\model` (reuse the `resolve_model` logic from the multi-model follow-up, or bind via `model: "name"`).
    - `dispatch.rs`: `statement_to_model_spec` gains an optional `prior: Option<PriorSpec>` and `solver: Option<SolverSpec>` (parsed from `\prior`/`\solver` body text — either direct JSON or a mini-grammar like `vacuum` / `bosons(0:2, 1:1)`). Fall back to `Vacuum`/`default()` when absent (backward-compatible).
    - `kernel_bridge.rs`: in `refresh`, collect `\prior`/`\solver` statements and pass them to `statement_to_model_spec` alongside the model.
    - **Tests:** `\prior(#1,#2, vacuum)` + `\model` → `ModelSpec.prior == Vacuum`; `\prior` with `bosons(0:1)` → `PriorSpec::Bosons(vec![(0,1)])`; `\solver(#1,#2, krylov_dim: 8)` → `SolverSpec.krylov_dim == 8`; missing `\prior` → vacuum (backward compat).
16. **Fix velysterm workspace CI (gap §8).** `cargo test --workspace --all-targets` is red because two upstream-vendored `velyst` examples (`editor`, `terminal`) reference removed APIs (`VelystFuncBundle`/`VelystSourceHandle`, E0422/E0425). The project's own crates build and test green. Fix: either port the examples to the current `velyst` API, or exclude them via `default-members` / `[workspace] exclude` so `cargo test --workspace` is green. This unblocks the velysterm CI (`rust.yml`) — currently red-on-arrival.
17. **Clean remaining clippy lints in `mathed_core`.** Two `single_range_in_vec_init` warnings remain: `transform.rs:566` (`vec![1..1]`) and `wordnav.rs:291` (`vec![2..8]`). Both are test-only. The `doc.rs` `unnecessary_sort_by` lint was already fixed in this session. A clean `cargo clippy --workspace --all-targets -- -D warnings` closes the lint baseline gap §8 for the project's own crates.
18. **Translator errors → `RepairHint` mapping.** Today translator failures surface as `KernelResult::Error { code_name: "translator" | "translator-json" | "translator-kind", message }` — a plain string with no structured `RepairHint`. The UK-#### protocol has a rich hint system (`HintKind::{ReplaceValue, SetParam, ReduceScope, IncreaseLimit, UseAlternativeOp}`) that could suggest concrete fixes:
    - `TranslateError::Eval(msg)` → `RepairHint { kind: ReplaceValue, target: "translator", suggestion: "fix Typst syntax error: {first line of msg}" }`.
    - `TranslateError::NotString` → `ReplaceValue` suggesting `json.encode(...)` as the return.
    - `DispatchError::Json(msg)` → `ReplaceValue` with the serde error (e.g. "missing field `kind`").
    - Missing named model → `ReplaceValue` suggesting valid model names in scope.
    This completes the Zero-language agent surface: every failure the user/agent can trigger from the editor carries a machine-readable repair suggestion, not just a message string.
19. **Benchmarks (P3 #13, not started).** A `cargo bench` suite (criterion) for the SIRK solve + Gram-whiten + reconstruct path. Key scenarios: harmonic-oscillator energy convergence vs Krylov dim, Gram whitening cost vs matrix size, reconstruct cost vs w_sequence length, translator eval cost vs body length. This is where a subtle numerical regression would otherwise hide — pass/fail tests catch crashes, not slowdowns or precision drift.
