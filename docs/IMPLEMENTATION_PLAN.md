...
**The remaining v2 frontier items** (P8 P7 Typst-math compiler, P8 P8 belief propagation, P9 P14 `mathed_mini` Step 4, P9 P15 stale `velyst` examples, P10 P16 real-dataset scaling, P10 P17 Lindbladian dynamics, P10 P18 Millennium-Prize mass-gap demo) are the documented next steps; each is a research-grade project of its own and will be tracked as its own revision when picked up. (P8 P9 `bayes_update_module` v2 — the Karcher-mean posterior estimate — is DONE in rev 19.)

## Next steps to improve (post-rev 19)

> **The v1 + v2 algorithm is closed and the closed algorithm is now productionized** (rev 18–19, 11 of 18 P7–P10 items done). The remaining 7 items are **research-grade (P8.7/P8.8/P10.16–P18), cross-repo (P9.14/P9.15), or single-day follow-ups (P9.15)**. The next rounds of work are no longer "implement the algorithm" nor "productionize" but **scale, validate empirically, and complete the user-facing surface**.
>
> **The recommended next revision is P7 P2 (real-data scaling, ongoing research)** — extending the MNIST 8×8 baseline (P7.3) to CIFAR-10 32×32, ImageNet 64×64, and cross-domain datasets (CFD, Ising-model critical snapshots). This is the central scientific claim of the QFM-TSR pipeline and has no size cap.
>
> **P8.7 (Typst-math compiler), P8.8 (belief propagation), P9.14 (mathed_mini Step 4), P9.15 (stale velyst examples), P10.17 (Lindbladian), and P10.18 (Millennium-Prize demo)** are the other documented next steps. Each is a research-grade project of its own and will be tracked as its own revision when picked up. (P8.9 — Karcher-mean posterior estimate — is DONE in rev 19.)
...
15. **Port the stale `velyst` examples** (P6 C7, ~1 day, **frontend**, cross-repo). Port `editor`/`terminal` to the current `velyst` API (removed `VelystFuncBundle`/`VelystSourceHandle`); retire the `upstream-stale-examples` feature gate.
...
16. **Real-dataset validation of QFM-TSR** (P7 P2, ongoing research; **next recommended revision**). Extend the P7.3 MNIST 8×8 baseline (65 training + 5 held-out digits, generated cosine sim ~0.6, Bayesian update overlap ~0.5) to (a) larger image datasets (CIFAR-10 32×32, ImageNet 64×64), (b) cross-domain datasets (CFD flow fields, 2D Ising-model critical snapshots, gravitational waveforms), and (c) the Bayesian-update posterior quality on adversarial / partial observations. Each scale-up is a separate P10.P16.X revision; the work is unbounded.
17. **Open-system / Lindbladian dynamics solver** (documented as out of scope in the P6 Future roadmap header; ~1 month, **research**). The QFM Fokker-Planck transport of QMF.tex §2.3 is currently approximated as a coherent Rabi oscillation. A full Lindbladian `e^{(H - i Σ L_k^dag L_k / 2) t}` solver is the natural next step for a true generative flow that mixes modes, not just oscillates. The Hermitian off-diagonal QFM coupling (`qfm_hamiltonian_offdiag`, gap §26) already exists as the seed for this work. Implementation path: (a) add a `L_k` Lindbladian jump operator set to `qfm`; (b) build a Lindbladian superoperator matrix `L = -i H ⊗ I + i I ⊗ Hᵀ + Σ (L_k ⊗ L_k - ½ L_kᵀ L_k ⊗ I - ½ I ⊗ L_kᵀ L_k)`; (c) feed through the same SIRK pipeline with a different reduced solver; (d) add a `qfm::bayes::lindbladian_evolve` function for non-Hermitian priors. The existing Born rule + Hermitian SIRK pipeline is the special case `L_k = 0`.
...
These are explicitly **not** pursued (the v1 + v2 + rev 18 plan records them as out of scope):
