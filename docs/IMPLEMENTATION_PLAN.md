...
- **Git state (2026-06-27, rev 6):**
  - unfer HEAD `b905178` (docs: mark P5 #31 done) on `main`. **Uncommitted:** `docs/IMPLEMENTATION_PLAN.md` (this revision). Needs commit + push.
  - australVM HEAD `b791ca88` (P5 #32: hot-swap smoke test) on `master`. Needs push.
  - velysterm HEAD `1c44f6b` (P5 #29: result_panel_markup footer) on `gitbutler/workspace`. Needs push.
- **Progress checklist:**
...
32. ~~**Hot-swap module testing.**~~ **DONE (2026-06-27, australVM `b791ca88`).** `hotswap_tests::hotswap_rejects_invalid_cell_id` in `cranelift/src/lib.rs` verifies `au_cell_swap` (the Rust wrapper around `cell_loader.c:cell_swap`) is linked correctly and guards out-of-range IDs (0, u64::MAX) by returning `false` without touching the table — the guard path that would prevent a dangling hot-swap. cranelift now passes **10 tests**. Full end-to-end hot-swap (load cell V1 → run → swap to V2 → run again) requires a `CellDescriptor` from a compiled `.so` (the C scheduler path) and is documented as a future shell-level test; the Rust→C linkage is now verified.
