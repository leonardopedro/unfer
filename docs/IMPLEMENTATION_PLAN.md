...
24. **Bevy frontend parity: inline annotations.** The Bevy `mathed` frontend's `draw_overlay` only renders colored underlines (green solid / red dashed) next to `\prob` spans — it does **not** pass `result_annotations()` into `TransformOptions`, so the green `= 0.4231` / red `code_name` text that the mini frontend shows is missing. Fix: add a `result_annotations()` passthrough to `kernel_sys::KernelBridge`, and set `annotations: kernel_bridge.result_annotations()` in the `TransformOptions` built in `main.rs`'s `sync_blocks` (currently `..Default::default()`). Low risk — the mini frontend already proves the path works.
...
28. **Translator authoring UX.** The translator panel (P3 #10) is functional but bare: the expanded panel shows raw Typst code with no syntax highlighting, and translator errors are only visible as a red `code_name` annotation on dependent `\prob`s. Add: (a) basic syntax highlighting in the expanded panel (Typst keywords, strings, comments); (b) inline translator error display — when a translator fails, show the error message (not just the code name) in the panel itself. Medium risk — touches the transform/render pipeline.
29. **`result_panel_markup()` wiring.** `KernelBridge::result_panel_markup()` produces a `#raw` block listing all prob results, is tested, but is **not displayed** by either frontend. The mini frontend uses only inline annotations (`result_annotations()`). Consider wiring the panel as a collapsible footer below the document (the `layout_doc_with_footer` API exists and is tested) for users who want a results summary rather than inline marks.
