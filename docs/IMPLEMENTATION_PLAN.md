...
9. **CPS-JIT backend: record/cross-module support — LARGELY RESOLVED (2026-06-26).** The experimental safestos CPS-JIT originally executed only `uk_*` foreign calls. Found and fixed four backend bugs so the linear `Model` wrapper now **runs end-to-end through the JIT** (`DemoModule` calls `kernelVersion`, then cross-module `wrapModel`→`freeModel`, the latter destructuring the linear record to recover the handle; `run_demo.sh` shows `Execution result: 1`):
   ...
   **Remaining (still open):** (a) **string/byte-buffer constants** are not lowered (`StringLit` → `0`), so a module still cannot build a JSON spec buffer to pass to `uk_model_create`; the full `uk_model_create`→`uk_evolve`→`uk_event_probability` chain from Austral remains blocked on this. (b) **Multi-field record sizing**: non-generic records reach the lowering as `MRecord` (keyed by `decl_id`) which the layout pass ignores, so `__record_new` under-allocates (default 8 bytes) and multi-field records corrupt the heap; single-field records (like `Model`) are unaffected. Both are the next backend items.
