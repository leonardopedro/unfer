...
**The remaining v2 frontier items** (P10.16.4+ cross-domain datasets, P10 P18 Millennium-Prize mass-gap demo) are the documented next steps; each is a research-grade project of its own and will be tracked as its own revision when picked up. (P8 P7 Typst-math compiler, P8 P8 belief propagation, P8 P9 `bayes_update_module` v2 — the Karcher-mean posterior estimate —, P9 P14 `mathed_mini` Step 4, P9 P15 + P9.15.1 stale `velyst` examples port, P10.16.1 CIFAR-10 synthetic baseline, P10.16.2 real CIFAR-10 fixture, P10.16.3 rank-truncation, and P11.24 Zenodo-Loro module are all DONE in rev 19/20/21/22/23/24/25/26. P10 P17 Lindbladian dynamics is excluded by user decision 2026-06-30.)

## Next steps to improve (post-rev 26)

> **The v1 + v2 algorithm is closed, the closed algorithm is now productionized, the velysterm upstream typst 0.15 / KanvaSink / kanva_typst update is merged, the mathed_mini Step 4 (caret blink + a11y + click-to-place-caret + kernel_client wiring) is formalized with tests, the 3 deleted velyst examples are re-introduced and ported to the velyst 0.15 API, the first CIFAR-10 16×16 real-data scaling baseline is shipped (P10.16.1 synthetic), the real CIFAR-10 fixture is committed (P10.16.2), the rank-truncation extension (P10.16.3) removes the O(K_2³) cubic wall enabling d=1024, and the Zenodo-Loro incremental persistence module is implemented (P11.24)** (rev 18–26, 17 of 18 P7–P10 items done + P11.24 of the P11 items). The remaining items are **research-grade (P10.16.4+ cross-domain / P10.18)** or **planned cross-repo (P11.19–P11.23)**. The next rounds of work are no longer "implement the algorithm" nor "productionize" but **scale empirically, complete the user-facing surface, and integrate the external modules**.
>
> **The recommended next revision is P10.16.4+ (cross-domain datasets)** — apply the QFM-TSR pipeline to non-image domains using the rank-truncation path (`max_rank: Some(r)`, `krylov_dim << K_2`): CFD flow fields, 2D Ising-model critical snapshots, gravitational waveforms. Each is an independent empirical sub-revision. Alternatively: **(P10.18)** Millennium-Prize mass-gap demonstration on l=3 4D SU(2) Yang-Mills (~2-3 months research). **(P11.19–P11.23)** external module integrations are planned cross-repo items; P11.24 (Zenodo-Loro, DONE rev 25) was the first P11 item actually implemented.
>
> **P10.18 (Millennium-Prize mass-gap demo on l=3 4D SU(2) Yang-Mills)** is the other documented next step. It is a research-grade project of its own and will be tracked as its own revision when picked up. (P8.7 — Typst-math compiler, P8.8 — chain belief propagation, P9.14 — mathed_mini Step 4, P9.15.1 — deleted velyst examples port, P10.16.1 — CIFAR-10 16×16 synthetic baseline, P10.16.2 — real CIFAR-10 fixture, P10.16.3 — rank-truncation, and P11.24 — Zenodo-Loro module — are DONE in rev 19/20/21/22/23/24/25/26.)
