...
3. ~~**Austral handle is plain `Int64` (S12 v1 choice).**~~ **RESOLVED at the type level (2026-06-26, P2.7).** `UnferKernel` now exposes a linear `Model` wrapper (`wrapModel`/`modelHandle`/`freeModel`) so `uk_model_free` is a compile-time obligation: the `LeakDemo` negative gate in `run_demo.sh` confirms a module that wraps a handle and forgets to free it fails to compile with a **Linearity Error**. **Caveat:** the raw `Int64` C ABI functions remain (the wrapper is built on them), and the wrapper cannot yet *run* through the CPS-JIT — that backend doesn't lower record-destructuring bindings or cross-module non-foreign calls, both of which `freeModel` needs (see gap §9). The linear guarantee is enforced by the Austral typechecker regardless of backend.
...
9. **CPS-JIT backend can't run the linear `Model` wrapper or buffer-passing (NEW, found 2026-06-26 during P2.7).** The experimental safestos CPS-JIT executes the `uk_*` foreign calls fine but does **not** lower two constructs the richer module surface needs: (a) record-destructuring `let` bindings (`let { handle } := model;` → `Undefined variable: handle`) and (b) cross-module calls to non-foreign Austral functions (`Call to unknown function: UnferKernel::wrapModel`). It also has no string/array-constant lowering, so a module cannot build a JSON spec buffer to pass to `uk_model_create`. Consequences: the linear `Model` wrapper (P2.7) typechecks and enforces free at compile time but can't *run* in-JIT; and the full `uk_model_create`→`uk_evolve`→`uk_event_probability` chain can't be driven from an Austral module yet (the `DemoModule` positive path stays at `uk_version`). Fix: extend `australVM/safestos/cranelift/src/cps.rs` codegen for record-destructure bindings, cross-module function linking, and byte-buffer constants — the single highest-leverage backend investment for the module mechanism.
...
7. ~~**Austral linear handle wrapper (S12 upgrade).**~~ **DONE (2026-06-26).** Added a linear `Model` record + `wrapModel`/`modelHandle`/`freeModel` to `UnferKernel.aui/.aum`; `freeModel` is the sole consumer and calls `uk_model_free`, so the type system now forces every `Model` to be freed exactly once. Proven by a **tested negative gate**: `demo_module/src/LeakDemo.aum` leaks a `Model` and `run_demo.sh` asserts it fails compilation with a *Linearity Error*. **Scope note (the plan's "if it fights the compiler subset" clause):** the linear wrapper typechecks cleanly but does **not run** through the CPS-JIT — that backend lacks record-destructure bindings and cross-module non-foreign calls. So `DemoModule` (the runnable JIT demo) still calls only `kernelVersion`; the wrapper's value here is the compile-time guarantee, which holds for any module regardless of backend. Closing the JIT gap is gap §9.
