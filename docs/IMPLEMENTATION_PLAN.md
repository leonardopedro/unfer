...
**The remaining v2 frontier items** (P10.16.3+ rank-truncation and cross-domain datasets, P10 P18 Millennium-Prize mass-gap demo) are the documented next steps; each is a research-grade project of its own and will be tracked as its own revision when picked up. (P8 P7 Typst-math compiler, P8 P8 belief propagation, P8 P9 `bayes_update_module` v2 — the Karcher-mean posterior estimate —, P9 P14 `mathed_mini` Step 4, P9 P15 + P9.15.1 stale `velyst` examples port, P10.16.1 CIFAR-10 synthetic baseline, P10.16.2 real CIFAR-10 fixture, and P11.24 Zenodo-Loro module are all DONE in rev 19/20/21/22/23/24/25. P10 P17 Lindbladian dynamics is excluded by user decision 2026-06-30.)

## Next steps to improve (post-rev 25)

> **The v1 + v2 algorithm is closed, the closed algorithm is now productionized, the velysterm upstream typst 0.15 / KanvaSink / kanva_typst update is merged, the mathed_mini Step 4 (caret blink + a11y + click-to-place-caret + kernel_client wiring) is formalized with tests, the 3 deleted velyst examples are re-introduced and ported to the velyst 0.15 API, the first CIFAR-10 16×16 real-data scaling baseline is shipped (P10.16.1 synthetic), the real CIFAR-10 fixture is committed (P10.16.2), and the Zenodo-Loro incremental persistence module is implemented (P11.24)** (rev 18–25, 16 of 18 P7–P10 items done + P11.24 of the P11 items). The remaining items are **research-grade (P10.16.3+ rank-truncation and cross-domain / P10.18)** or **planned cross-repo (P11.19–P11.23)**. The next rounds of work are no longer "implement the algorithm" nor "productionize" but **scale empirically, complete the user-facing surface, and integrate the external modules**.
>
> **The recommended next revision is P10.16.3 (rank-truncation extension)** — implement the documented "rank truncation via SVD on the W basis" extension point (`QMF.tex §11`) to handle d > 256 efficiently without the W_prob cubic-scaling wall (needed for CIFAR-10 32×32, d=1024). The alternative next steps: **(P10.16.4+)** cross-domain datasets (CFD flow fields, 2D Ising critical snapshots, gravitational waveforms); **(P10.18)** Millennium-Prize mass-gap demonstration on l=3 4D SU(2) Yang-Mills (~2-3 months research). **(P11.19–P11.23)** external module integrations are planned cross-repo items; P11.24 (Zenodo-Loro, DONE rev 25) was the first P11 item actually implemented.
>
> **P10.18 (Millennium-Prize mass-gap demo on l=3 4D SU(2) Yang-Mills)** is the other documented next step. It is a research-grade project of its own and will be tracked as its own revision when picked up. (P8.7 — Typst-math compiler, P8.8 — chain belief propagation, P9.14 — mathed_mini Step 4, P9.15.1 — deleted velyst examples port, P10.16.1 — CIFAR-10 16×16 synthetic baseline, P10.16.2 — real CIFAR-10 fixture, and P11.24 — Zenodo-Loro module — are DONE in rev 19/20/21/22/23/24/25.)
...
16. **Real-dataset validation of QFM-TSR** (P7 P2, ongoing research). **P10.16.1 DONE in rev 24 (CIFAR-10 16×16 synthetic fixture, `qfm/tests/cifar10_validation.rs`, 4 tests; also fixed the SIRK shift overflow that prevented krylov_dim > 88). P10.16.2 DONE in rev 25 (real CIFAR-10 16×16 fixture: `qfm/scripts/make_cifar10_fixture.py` + `qfm/testdata/cifar10_16x16_{256training,8heldout}.json` + `qfm/tests/cifar10_real_validation.rs`, 4 real-data tests).** Remaining sub-revisions: **(P10.16.3)** implement the rank-truncation extension point (`QMF.tex §11`) to enable d=1024 (CIFAR-10 32×32) without W_prob cubic-scaling bottleneck; **(P10.16.4+)** cross-domain datasets (CFD flow fields, 2D Ising-model critical snapshots, gravitational waveforms). Each sub-revision is independent; the work is unbounded.
...
- **Progress checklist:**
  ...
  - [ ] **P10.16.3** — Rank-truncation extension (`QMF.tex §11`): SVD on W basis, remove W_prob cubic-scaling wall, enable d=1024 (CIFAR-10 32×32)
  - [ ] **P10.16.4+** — Cross-domain datasets (CFD flow fields, 2D Ising critical snapshots, gravitational waveforms) — ongoing research
  ...
  - [ ] **P11.19** — `pattern_unfer` persistent CRDT session memory (adapts `../pattern` `pattern_memory` MPL + `../pattern-main` AGPL; lives **inside `../pattern`**; bridges via `uk_snapshot`/`uk_restore` + NDJSON; planned, cross-repo)
  - [ ] **P11.20** — `arctic_authority` threshold-signed collective authority (adapts `../dynamic-arctic` MIT; australVM `AuthorizationEngine` + signed `uk_observe`; planned, cross-repo)
  - [ ] **P11.21** — `hayagriva` bibliography/citation in the math editor (adapts `../hayagriva` MIT/Apache; direct velysterm dep, e.g. `crates/mathed_biblio`; planned, cross-repo)
  - [ ] **P11.22** — `unfer_edge` Pingora-based security-first proxy module **inspired by** `../zentinel` (zentinel itself NOT integrated — redundant; reuse only its design + the fork's `ai-gateway`/`data-masking`/`secret-inject`/`wasm-allowlist` ideas; direct `pingora` dep; fronts the `unfer_agent` interface; planned)
  - [ ] **P11.23** — `unfer_nixvm` GPU-shared Nix execution sandbox (adapts `../cloud-hypervisor-build` Apache/BSD: cloud-hypervisor 50.0 + spectrum0 GPU patches + virtiofs `/nix` store sharing for bidirectional host↔VM package transfer; reproducible CUDA pin = clean P9.12; inspiration only from `../claurst` GPL-3.0; planned, cross-repo)
