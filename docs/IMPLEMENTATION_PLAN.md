...
## Current status (updated 2026-07-14, rev 37 v3)

**Rev 37 v3 (this update) — diffusion Hamiltonian replaces per-order dressed-vacuum sum.** The per-order dressed-vacuum projector `H = Σ_o λ_o |0̃_o⟩⟨0̃_o|` (rev 31–36) is removed for the text path. The new generator is the diffusion Hamiltonian
```
H = λ₀·|c₀⟩⟨c₀| + λ₁·Σ_{(i→f)} (|f⟩⟨i| + |i⟩⟨f|)
```
where `|c₀⟩` is the outer vacuum (uniform in the Fock-space input basis with R partitions of the infinite-dimensional hypersphere, each with amplitude √R) and the sum runs over consecutive training-window mode transitions. Inner products use the geometric mode-overlap metric `G_{mn} = δ_{mn} + γ·(1-δ_{mn})` with `γ = 1/R`. The Krylov starting vector has amplitude √R. The W-rank-1-2 pathology (rev 35) was caused by the old per-order projector sum having rank bounded by n_orders (≤ 4) with all projectors nearly parallel to the Fock vacuum. The new Hamiltonian has O(M) distinct transition terms (M = total training windows), giving the Krylov basis a much richer off-diagonal structure. Train + eval results with the new Hamiltonian are pending; all tests pass (149 passed, 2 ignored). See `docs/QFM_TEXT_STATUS.md` §"Rev 37 v3 architecture changes" for full rationale and changed files.

**Rev 36 (prev) — QFM-Text Stage 6/7 completion: full hyperparameter sweep, `QFM.tex` QFM-Text subsection, 4-epoch training in flight.** Completes the Stage 6 acceptance criterion (sweep over `t × λ × discount`, 30 configs) and the Stage 7 documentation deliverable (new `QFM.tex` subsection at the end of the TSR section, with the multi-projector generator equation, the α→ε normalization, the Born-rule token head, the honest-scope paragraph, the measured numbers table, and a `rev 32--35` entry in the `\date`). The Stage 6 sweep walks the plan-specified $5\times3\times2=30$ configurations: $t\in\{0.25,0.5,1.0,1.5,2.0\}$ × $\lambda\in\{\mathrm{uniform}, \propto\mathrm{order}, \propto1/\mathrm{order}\}$ × $\mathrm{discount}\in\{0.5,0.75\}$. The per-$\lambda$ cost is ~1-2 s (one `compile_channels` round-trip via the new `QfmTextModel::rebuild_with_lambda(&[f64]) -> QfmTextModel` method, which uses `as_accumulator()` → `from_accumulator` to re-build the Krylov pipeline with new per-order $\lambda_o$ without re-accumulating the histograms). The result: the W-rank-1 pathology (rev 35) makes the Krylov evolution a near-identity, so the sweep produces a tight range of ppls (395.5--396.1 on the 5k-token held-out slice) and **no sweep configuration beats the classical n-gram baseline** (the negative result is reported in `docs/QFM_TEXT_STATUS.md` §"Hyperparameter sweep" per the Stage 6 acceptance contract). 4-epoch m=32 training is in flight (`qfm_text_runs/m32_4ep/`) to populate `metrics.ndjson` with 4 non-decreasing `n_windows` lines (the Stage 6 cadence requirement). Workspace test count: **323 → 324** (+1 from `rebuild_with_lambda_round_trip`).

**Rev 35 (prev) — QFM-Text debug pass: OOM root cause found, full-corpus training, W-rank degeneracy characterised, per-context fit measured.** This rev was a **second honest re-measurement** of the rev 33 numbers: (a) `accumulate_shards` had a `par_iter().collect()` building a `Vec<ChannelAccumulator>` of 245 per-shard accumulators, peaking at 33 GB on this 16 GB system. The OOM killer silently killed the rev 33 training mid-corpus, leaving behind "checkpoints" that had only been trained on a subset of shards. The 92.5 s/95.7 s wall times reported in rev 33+34 were from the killed processes, not from full 128M-token training. The fix is a sequential pass with one in-flight accumulator (`qfm_text/src/accumulate.rs:80`, peak = 1 shard). (b) Full-corpus re-training on the 128,102,894-token WikiText-103 train split: **m=8 wall 7m32s (451.9s), m=32 wall 7m47s (467.1s)**. (c) New `--diagnose` flag in `qfm_text_eval` prints the SVD rank of $W$ and $H_m$ on the loaded checkpoint: **W has rank 1-2, NOT the requested `m_shifts`**. m=8 → rank 2, m=32 → rank 1. The Gram-whitening of the SIRK Krylov basis finds only 1-2 linearly independent vectors in the (m+1)-dim space because the dressed-vacuum Hamiltonian `H = Σ_o λ_o |0̃_o⟩⟨0̃_o|` with `c_0 ≈ 1, ε_j ≈ ᾱ_j ≈ 5e-6` (per-mode weight, 210K active modes) gives four nearly-parallel `|0̃_o⟩ ≈ |0⟩`. **m=32 is WORSE than m=8** (rank 1 < rank 2): adding shifts only adds noise. (d) Per-context fit on the first 4123 contexts of train shard 0: mean KL(emp‖qfm) = **3.84 nats**, mean cosine = **0.38**, top-1 hit = **33%**, top-5 hit (≥3 overlap) = **8%** — **identical for m=8 and m=32** (the fit is dominated by the H structure, not by `m_shifts`). The QFM fails even on **fully deterministic** contexts (100% empirical mass on one token) where it puts 4-10% on a wrong token. (e) Honest numbers on the full 128M-token corpus: in-sample (train shard 0) QFM m=8 = **253.8**, m=32 = **237.2** vs baseline **182.7** (gaps 1.39×/1.30× — *wider* than rev 33's 1.16× because the baseline improved more from the extra data: 321.4 → 182.7 = 43% reduction vs QFM 32%); held-out (test split, 100K tokens) QFM m=8 = **420.8**, m=32 = **382.3** vs baseline **364.6** (gaps 1.15×/1.05×). (f) New `QfmTextModel::as_accumulator()` + `--baseline-from-checkpoint` flag in `qfm_text_eval` to derive the n-gram baseline from the model's stored `mode_hists` + unigram (fast: <1 s vs ~7 min for a corpus re-derivation). Held-out eval time analysis: 4 ms/token (m=8, W rank 2) or 0.5 ms/token (m=32, W rank 1); full 128M-token in-sample is **~140 hours CPU** (infeasible without GPU acceleration of `decode_sketched`). The "QFM is essentially a classical n-gram" finding of rev 33 is now quantified and traced to the W-rank-1 degeneracy. `qfm_text` workspace test count: **38** (+1 from `as_accumulator` round-trip). Full report in `docs/QFM_TEXT_STATUS.md` rev 35.

**Rev 34 (prev) — QFM.tex consistency audit: the exact projector is restored as the only compiled QFM generator; false article claims corrected.**

**Rev 34 (this update) — QFM.tex consistency audit: the exact projector is restored as the only compiled QFM generator; false article claims corrected.** An audit of the code against `QFM.tex` found that the rev 33 interim had switched the QFM-Text compile to the *diagonal* surrogate (eq. Hdiag, `QfmPipeline::compile_alphas` → `qfm_hamiltonian`), whose doc comment mislabeled it "the QFM.tex-spec'd structure". QFM.tex says the opposite: the diagonal generator's Born populations are stationary (it is not a flow; §"Scope: the diagonal surrogate and the exact off-diagonal projector"), and since rev 31 the exact dressed-vacuum projector is the *only* off-diagonal generator. The diagonal form also doesn't fix the small-Krylov problem it was adopted for: a diagonal generator's Krylov dimension is the number of *distinct* eigenvalues, which count-degenerate ᾱ_j collapse. Changes: (1) `compile_alphas` and the dead `ChannelCompileError` (whose Σα²≤1 message was mathematically wrong for the eq. Htomo normalization — the ᾱ are unbounded) are **deleted**; `QfmPipeline::compile_channels` is reimplemented as the canonical text compile on `qfm_hamiltonian_hierarchical_projectors` (one exact rank-1 `ProjectOnto` term per order, eq. Htomo normalization per group, per-mode ᾱ_j preserved — the old deprecated wrapper had silently discarded them), and `QfmTextModel::from_accumulator` routes through `build_channel_groups` → `compile_channels`. (2) **QFM.tex corrections:** the Phase-3 claim "W is real by construction" was false — the SIRK shifts ζ_k = −ik/m are imaginary, so the whitened basis is complex; the real-part W_prob contraction evaluates to ½(|⟨a|Wc₁⟩|² + |⟨a|W̄c₁⟩|²) (Born population + a spurious conjugate-basis term; exact only for real W). The paper now states this, documents that the token head computes the exact Born rule via `decode_sketched` while the image path keeps the pre-projected observable form, and carries a rev 34 note in the QFM-Text item that the rev 32–33 perplexity numbers must be re-run against the restored generator before being quoted. (3) Comment/doc fixes: `qfm_hamiltonian_hierarchical_projectors` (no Σα²≤1 panic; eq. Htomo normalization, not c₀=√(1−Σα²)), `optimal_coefficients` (closed form; no Riemann sum is performed), `rank_truncate_w_h` (real-part SVD is a documented heuristic; W is complex in general). All 323 workspace tests pass; `qfm_text` is 35 unit + 2 integration.

**Rev 33 (prev) — three critical bugs found and fixed.** (1) `context_modes` (qfm_text/src/model.rs:570) was returning raw context tokens instead of hashed mode indices, so the Krylov read the wrong W rows and `marginalize` looked up empty histograms; the QFM distribution exactly equalled the unigram. (2) `decode_sketched` (qfm/src/pipeline.rs:612) was using a "sketched" Born formula `p̃ = W_prob · Re(vec(c c†))` that has destructive cancellation in the off-diagonal ρ elements when W has small imaginary parts (numerical noise from Gram-Schmidt), producing `p̃[m] = 0` for all m; replaced with the actual Born rule `p̃[m] = |⟨m|W|c⟩|²`, which is always non-negative. (3) The W matrix is rank 1-3 with 99.98% zero rows because the SIRK Krylov sequence has only m+1 elements, so the basis can only span a small number of modes; the dressed-vacuum Hamiltonian has rank ≤ n_orders+1, so the Krylov dim is fundamentally small. **The QFM-Text architecture as implemented is functionally equivalent to the classical n-gram baseline** (gap 1.06-1.16× on held-out, within numerical noise). The user's suggestions of "smaller Krylov" or "larger vacuum projector weight" don't help because the Krylov is already rank 1-3 and is essentially bypassed by the Renormalize fallback. New held-out evaluation via `--eval-manifest` flag: QFM in-sample 371.7, held-out 501.7, baseline in-sample 321.4, held-out 474.8. Workspace test count: **302 → 323 (+21: +2 qfm_text unit + 1 incontext, and the GPU `fock_sirk` tests that now run by default with `--features fock_sirk/cuda`)**.

**Rev 32 (prev) — ships QFM-Text: train the QFM.tex architecture on token streams using HRM-Text's data + training process.** New workspace member `qfm_text/` (29 unit + 2 integration tests; `cargo test -p qfm_text` green in <1 s; end-to-end on the WikiText-103 test split completes in <1 s per epoch on CPU). The new crate implements all 7 stages of `docs/QFM_TEXT_HRM_PLAN.md`: (Stage 0) `scripts/prepare_corpus.py` downloads + tokenizes WikiText-103 into little-endian u32 shards + a JSON manifest; (Stage 1) memory-mapped `Shard` reader + `WindowIter`; (Stage 2) hashed Level-2 `OrderHasher` + streaming `ChannelAccumulator` with the FSDP-analog `merge`; (Stage 3) `qfm_hamiltonian_hierarchical_projectors` in `nested_fock_algebra` (the new `H = Σ_o λ_o |0̃_o⟩⟨0̃_o|` generator — one exact `ProjectOnto` term per order); (Stage 4) `QfmPipeline::compile_channels` + `encode_modes` + `decode_sketched` + the `QfmTextModel` Born-rule token head; (Stage 5) perplexity + sample + classical `NgramBaseline` + the Quantum Bayesian Update as in-context adaptation; (Stage 6) the `qfm_text_train` + `qfm_text_eval` binaries (TOML config, NDJSON metrics, hyperparameter sweep); (Stage 7) this entry. **The end-to-end smoke run on WikiText-103 (266,932 tokens, 16k BPE, 2-order / K₂=8 192 / max_rank=4 model, 2 epochs) is reported in `docs/QFM_TEXT_STATUS.md` as a negative result: QFM ppl = 1395.6 vs. classical n-gram baseline ppl = 54.8 (and unigram ppl = 269.4).** The hyperparameter sweep (`t ∈ {0.25, 0.5, 1.0, 1.5, 2.0} × discount ∈ {0.5, 0.75}`) does not change the winner. The dominant failure mode is the uniform-superposition encode at low `max_rank=4` producing a `p̃` too uniform to discriminate modes; the status doc records the next-step fixes (rank 64, full corpus run, better encode). All 271 baseline workspace tests still pass. Workspace test count: **271 → 302 (+31: 29 qfm_text unit + 2 qfm_text integration)**.

**Rev 31 (prev) — removes the O(ε) truncation: the exact projector is now the only off-diagonal QFM generator (user decision 2026-07-05).** A new `Operator::ProjectOnto(Arc<QuantumState>)` variant in `nested_fock_algebra` implements the exact rank-1 projector `|0̃><0̃|` via the shortcut `H|s> = <0̃|s>·|0̃>` (O(components) per matvec — the "exploit the rank-1 structure" extension point from `QFM.tex`, now the primary path instead of the O(M²) frame expansion). `qfm_hamiltonian_mehler_projector` now builds a single `ProjectOnto` term; `qfm_hamiltonian_offdiag` / `qfm_hamiltonian_offdiag_localized` (the P5 #26 truncations) and their `qfm_mehler_offdiag` / `qfm_mehler_offdiag_localized` builtins are **deleted**, replaced by the exact `qfm_mehler_projector_localized(points, epsilons, scale)`. The TSR pipeline's `build_flow_hamiltonian(alphas, k2)` now normalizes the dressed vector `|vac> + Σ α_j|x_j>` and returns the exact projector onto it (ε_j = α_j/√(1+Σα²), c₀ = 1/√(1+Σα²)); truncated-dynamics tests (Rabi ω=√(1+4α²)) were replaced by closed-form projector-oscillation tests (`P_j(t) = 4sin²(t/2)c₀²ε_j²`, coherent return at t=2π). Historical entries below that describe `qfm_hamiltonian_offdiag` (gap §26, F-workstream notes) record the pre-rev-31 state.

**Rev 30 — ships P10.16.7 (cross-domain scaling to d=1024 of all three rev 29 cross-domain fixtures), the first item from the QFM.tex post-rev 29 "next steps" list.** The workspace test count is **271** (was 259 in rev 29, +12 from the new test file). The new file `qfm/tests/d1024_cross_domain_scaling.rs` consolidates the three rev 29 fixtures (CFD L=16→32, Ising L=16→32, GW d=256→1024) at d=1024, each with the same 4-test battery (compile / generate-finite / bayes-update / no-explosion). All 12 tests use the P10.16.3 rank-truncation path (`krylov_dim=32, max_rank=Some(16)`) with `K_2 = max(M, d) = 1024` (the M=64 < K_2=1024 case forces the lossy path, exactly as the synthetic `rank_truncation.rs` test exercises). The CFD L=32 trajectory is shared across the 4 CFD tests via `std::sync::OnceLock` (the L=32 solver takes ~10 s to generate the 64-snapshot trajectory, so deduplicating saves ~30 s). All 12 tests pass with `cargo test -p qfm --test d1024_cross_domain_scaling` in ~20 s on CPU with parallel threads. The real correctness signal at d=1024 is the **Bayesian overlap** (`|<sample|c_obs>|²`) and the **finite-output** check, not the **generate-similarity** (the rank-2 path collapses to a single representative output when M/d=1/16, 4× the sparsity of the d=256 fixtures). The GW d=1024 Bayesian overlap is [0.05, 0.77] for 4/8 held-out (the rank-2 PN family is captured well); the Ising d=1024 Bayesian overlap is [0.70, 0.78] for 2/8 (the other 6 hit either rank-collapse or the S_2-hash edge case). See "P10.16.7 (rev 30)" subsection in `QFM.tex` for the per-test metrics + algorithm details. The "next steps" list now points to P10.16.8+ (richer cross-domain datasets: real-data CFD, full LIGO mode content, 3D Ising, or non-stationary distributions) and the multi-month continuum-limit mass gap proof.

**Rev 29 (prev) — ships P10.16.5 (2D Kolmogorov-flow CFD cross-domain validation) and P10.16.6 (post-Newtonian gravitational-waveform cross-domain validation), the two open items from the QFM.tex post-rev 28 "next steps" list.** The workspace test count is **259** (was 251 in rev 28, +8 from the two new test files; 4 tests each). Both new fixtures use the P10.16.3 rank-truncation path (`krylov_dim=32, max_rank=Some(16)`) since M=64 < K_2=256 forces the lossy path. They are genuinely different from the existing image fixtures (CIFAR-10 / Ising) — **CFD** is a real-PDE-generated continuous spatial field (vorticity in a 2D Navier–Stokes Kolmogorov flow, integrated by a hand-rolled radix-2 FFT pseudospectral solver with 2/3-rule dealiasing + RK4), and **GW** is a real-physics 1D time series (the dominant ℓ=2, m=2 mode of the GW strain from restricted 2PN TaylorT1 inspiral waveforms over a 64-mass-parameter bank). The GW test's high cosine-similarity (~0.72) reflects the rank-2 structure of the PN family (amplitude × phase), which the rank-truncated QFM-TSR captures well. The CFD test's cosine similarity (~0.36) and 0.18–0.77 Bayesian overlaps show the pipeline handles the spatially-correlated, time-dependent Kolmogorov flow. Both run in <10 s on CPU. See "P10.16.5 + P10.16.6 (rev 29)" subsection in `QFM.tex` for the per-test metrics + algorithm details.

**Rev 28 (prev) — ships all four remaining P11 external-module integrations: P11.20 (`arctic_authority`), P11.19 (`pattern_unfer`), P11.21 (`hayagriva` → `mathed_biblio`), and P11.23 (`unfer_nixvm`).** All five P11 items are now built; P11 is closed. Each is documented in full in its own numbered entry below (§P11); summary:

- **P11.20 — `arctic_authority`** (new crate inside australVM, `australVM/arctic_authority/`, MIT, depends on `../dynamic-arctic`): `ArcticAuthEngine` verifies a `DelegationCertificate` against Arctic's threshold Schnorr `arctic_core::verify`, then authorizes `(principal, action, resource)` by capability + expiry — a collective, *n*-of-*t* analogue of the existing single-policy `ManifestAuthEngine`/Cedar path. 9 unit tests. Bridged into `australVM/safestos/cranelift` as a new optional `arctic-auth` feature (`src/arctic_auth.rs`, `ArcticVmEngine: AuthorizationEngine`, `install()` wires it into `auth::set_auth_engine`), 1 more test; builds clean alone, with `default`, and combined. No zentinel/pattern/etc. code crossed — MIT↔MIT.
...
- **P11.23 — `unfer_nixvm`** (new subdirectory `unfer/unfer_nixvm/`, composes — does not vendor — `../cloud-hypervisor-build` Apache-2.0/BSD-3-Clause; UX inspiration only, never code, from `../claurst` GPL-3.0): `../unfer/flake.nix` gained `packages.x86_64-linux.unfer-ffi` (a reproducible `rustPlatform.buildRustPackage` derivation of the CPU-only `unfer_ffi` cdylib, built via a new `nixpkgs-unstable` flake input since the workspace's `edition = "2024"` needs a newer rustc than the CUDA devShell's pinned `nixos-23.05` ships — that devShell/pin is untouched). **Actually built** in this session (`nix build .#unfer-ffi` → a real `libunfer_ffi.so`/`.a`), not just written. `unfer_nixvm/flake.nix` layers a NixOS module installing that package on top of `../cloud-hypervisor-build`'s existing `configuration.nix` (taken as a `flake = false` path input, so upstream's virtiofs/GPU/agent-user module stays the single source of truth), producing `vm-perf-with-unfer`/`vm-sec-with-unfer` outputs — `nix eval` confirmed the whole composition is well-formed (reached real NixOS `system.build.toplevel` evaluation) before this sandboxed environment ran out of disk (`~17G` free was not enough for a full NixOS image closure) attempting `nix build`; that is an environment resource limit hit while verifying, not a defect. Also fixed a real bug in `../cloud-hypervisor-build/full-stack-vm-launch.sh`: `--shared-dir ../nix` resolved relative to the caller's cwd (not `$SCRIPT_DIR`) and pointed at a nonexistent directory rather than the host's actual `/nix/store` unless invoked from one specific cwd — changed to the absolute `/nix`, the fix that makes the "host store IS guest store" mechanism the plan describes actually true. Deliberately **not done**: actually booting `vm-perf`/`vm-sec` (requires `sudo`, real GPU/tap-network device access — left for the user to run by hand per `unfer_nixvm/README.md`); packaging `australVM`/`qfm` alongside `unfer_ffi`; a `cuda`-feature Nix package variant; and the `ro`→`rw` `/nix` mount security-boundary decision needed for genuine host↔VM bidirectional package transfer (documented as an open decision, not flipped unilaterally).

`arctic_authority` and `unfer_nixvm` were initially written as standalone directories with no git repo of their own (a quick way to get something building and tested), then — on the user's explicit instruction, since both are MIT-licensed — moved into the repos that actually consume them: `arctic_authority` into `australVM/arctic_authority/` (its only consumer is `australVM/safestos/cranelift`), `unfer_nixvm` into `unfer/unfer_nixvm/` (its `packages.*` output is entirely about the unfer kernel). Both were re-verified working at their new locations (`cargo test`/`cargo clippy` for `arctic_authority` + the `arctic-auth` cranelift feature; `nix eval` for `unfer_nixvm`) before being committed. `../cloud-hypervisor-build` itself (~930MB, full vendored `crosvm`/`cloud-hypervisor` upstream checkouts + a built `.deb`) was deliberately **not** moved or vendored into any git repo — instead, the small hand-authored recipe behind it (Nix/shell files, ~150KB) was copied to `australVM/cloud_hypervisor_vm/`, with a new `setup.sh` that regenerates an equivalent working tree from pinned upstream commits. `unfer_nixvm`'s flake still composes with whichever `cloud-hypervisor-build`-equivalent directory is on disk via a path input, regardless of which recipe produced it.
...
**All P10 v2-frontier items are now DONE as of rev 30** (P10.16.4 2D Ising cross-domain dataset, P10.16.5 2D Kolmogorov-flow CFD, P10.16.6 post-Newtonian gravitational-waveform bank, P10.16.7 d=1024 cross-domain scaling, P10.18 l=3 Yang-Mills mass-gap demo). (P8 P7 Typst-math compiler, P8 P8 belief propagation, P8 P9 `bayes_update_module` v2 — the Karcher-mean posterior estimate —, P9 P14 `mathed_mini` Step 4, P9 P15 + P9.15.1 stale `velyst` examples port, P10.16.1 CIFAR-10 synthetic baseline, P10.16.2 real CIFAR-10 fixture, P10.16.3 rank-truncation, P10.16.4 2D Ising, P10.16.5 Kolmogorov CFD, P10.16.6 gravitational waveforms, P10.16.7 d=1024 scaling, P10.18 l=3 mass-gap demo, and P11.24 Zenodo-Loro module are all DONE in rev 19/20/21/22/23/24/25/26/27/29/30. P10 P17 Lindbladian dynamics is excluded by user decision 2026-06-30.) The l=3 mass-gap demo is a lattice-scale empirical result, not the continuum-limit Millennium-Prize proof — but the checklist items as originally scoped are satisfied.
...
> - **Richer cross-domain datasets (P10.16.8+)**: the rev 30 P10.16.7 closed the d=1024 scaling. The natural next step is richer families: real-data CFD (a published DNS dataset rather than synthetic Kolmogorov forcing), full LIGO Open Data Center waveforms (the $\ell=2,3$ mode content rather than the TaylorT1 face-on approximation), 3D Ising (L=8 or L=10, d=512 or d=1000), or non-stationary distributions (a time-series of CFD snapshots with the QFM-TSR pipeline tracking the distribution as it evolves).
...
The closed v1 + v2 algorithm is now **productionized**: a system that can be **deployed, monitored, and extended** by an outside user. The 6 items (P7.1 through P7.6) are all done in rev 18 (commit `edbda89`); see the "P7 productionization (all 6 items done)" subsection in `QFM.tex §8.9` for the per-item implementation notes. Net effect on the workspace:
...
8. **`uk_belief_propagation` FFI symbol** (~1 week, **research**). The QFM.tex §8 algorithm does a full Bayesian update; a v2 system could add **iterated belief propagation** (message passing between observation nodes) as a faster alternative to HMC for product-of-likelihoods posteriors. This is a documented QFM.tex §11 extension point. Out of scope for P7, but a natural follow-on.
...
> **Status:** all five are **built** — P11.24 (Zenodo-Loro, rev 25), P11.22 (`unfer_edge`, rev 27), and P11.19/P11.20/P11.21/P11.23 (rev 28). P11 is closed. The hands-on tutorial in `docs/TUTORIAL.md` (§"External modules") walks through the intended data flow for each. **License summary:** P11.19 (`pattern_unfer`, `../pattern` MPL + `../pattern-main` AGPL) lives inside `../pattern` (new crate `crates/pattern_unfer/`) and crosses to unfer only over the `unfer_agent` NDJSON protocol boundary — no Cargo dependency either direction; P11.20 (`arctic_authority`, `../dynamic-arctic` MIT) is a new permissive crate living inside `australVM` (`australVM/arctic_authority/`) plus an optional `arctic-auth` feature in `australVM/safestos/cranelift`; P11.21 (`../hayagriva` MIT/Apache) is a direct velysterm dep (new crate `crates/mathed_biblio/`); P11.22 is a **new Pingora-based module inspired by `../zentinel`** (Apache-2.0) — zentinel itself is *not* integrated (redundant), and unlike the other four it lives entirely inside unfer as `unfer_edge/`; P11.23 (`unfer_nixvm`) composes `../cloud-hypervisor-build` (Apache-2.0/BSD, a new subdirectory `unfer/unfer_nixvm/` layers a NixOS module on top rather than editing its `configuration.nix` directly — only a one-line path bug in its launch script was fixed in place) and takes **inspiration only** from `../claurst` (GPL-3.0, copyleft — never vendored).
...
2. `QFM.tex` — the algorithm specification (Sections 1–4 for the core flow, Section 7 for the QFM-TSR pipeline, Section 8 for the Bayesian update + the rev 18 "Post-rev-17 productionization, validation, and infrastructure" subsection).
...
8. The 6 module demos (`demo_module/`, `qfm_module/`, `qfm_tomo_module/`, `demo_module/data_source/`, `bayes_update_module/`, `iterated_bayes_module/`) — each is a self-contained "executable documentation" of one slice of the architecture. Start with `qfm_tomo_module/`: it is the smallest end-to-end demo that exercises the F4 + F5 (online inference) + UK-4001 (authorization) + buffer protocol (drain result). Then `iterated_bayes_module/`: it exercises the full QFM.tex §7 + §8 pipeline in a loop (3 iterations of `uk_bayesian_update` + `uk_evolve`).
...
- **Progress checklist:**
  ...
  - [x] **P10.16.4** — 2D Ising-model critical snapshots (rev 27, first cross-domain sub-revision): `qfm/tests/ising2d_validation.rs`, L=16 (d=256) Metropolis MC at the exact critical temperature β_c = ln(1+√2)/2 (Onsager 1944), 64 training + 8 held-out configurations from independent seeds; uses the P10.16.3 rank-truncation path (`krylov_dim=32, max_rank=Some(16)`) since M=64 < K_2=256 forces the lossy path. 4 tests: compile, generate finite+correlated, Bayesian update on held-out, no-explosion. CFD flow fields and gravitational waveforms were the remaining P10.16.4+ sub-items, closed in rev 29 as P10.16.5 + P10.16.6 (next two entries).
  - [x] **P10.16.5** — 2D Kolmogorov-flow CFD cross-domain validation (rev 29): `qfm/tests/cfd_kolmogorov_validation.rs`, L=16 (d=256) 2D incompressible Navier–Stokes vorticity on a doubly-periodic box forced by $f(x,y) = \sin(k_f x) \cos(k_f y)$ ($k_f = 4$, $\nu = 0.05$), integrated by a hand-rolled radix-2 FFT pseudospectral solver with 2/3-rule dealiasing and RK4 time stepping. 64 training + 8 held-out snapshots captured every 20 time steps after a 50-step burn-in. Uses the P10.16.3 rank-truncation path. 4 tests pass in ~5 s: compile (rank=2, $K_2=256$, $d=256$), generate with cosine similarity ~0.36, Bayesian update with $|\langle sample | c_{obs} \rangle|^2 \in [0.18, 0.77]$, no explosion.
  - [x] **P10.16.6** — Post-Newtonian gravitational-waveform cross-domain validation (rev 29): `qfm/tests/gravitational_waveform_validation.rs`, restricted 2PN TaylorT1 inspiral waveforms ($\ell=2, m=2$, face-on, non-spinning, total mass $M \in [1, 4]M_\odot$ in geometric units, $d=256$ samples over $[0, 50M]$). The phase follows $\phi(t) = 2\pi f_0 t_c (1 - t/t_c)^{5/8}$ with $t_c = (5/256) \mathcal{M}_c^{-5/3} (\pi f_0)^{-8/3}$ and $\mathcal{M}_c = M / 2^{1/5}$ (matches LIGO's TaylorT1 to leading order). 64 training + 8 held-out waveforms. Uses the P10.16.3 rank-truncation path. 4 tests pass in ~1 s: compile, generate with cosine similarity ~0.72 (the rank-2 amplitude × phase PN structure), Bayesian update with $|\langle sample | c_{obs} \rangle|^2 \in [0.34, 0.61]$ for 4 of 8 held-out (the other 4 hit the same S$_2$-hash-to-unsupported-mode edge case as MNIST), no explosion.
  - [x] **P10.16.7** — d=1024 cross-domain scaling of all three rev 29 fixtures (rev 30): `qfm/tests/d1024_cross_domain_scaling.rs` consolidates CFD L=16→32, Ising L=16→32, and GW d=256→1024 into a single 12-test file (3 fixtures × 4 tests each). All 12 tests use the P10.16.3 rank-truncation path with `K_2 = max(M, d) = 1024` (the M=64 < K_2=1024 case forces the lossy path, exactly as the synthetic `rank_truncation.rs` test exercises). The CFD L=32 dataset is shared across the 4 CFD tests via `std::sync::OnceLock` (the L=32 solver takes ~10 s to generate the 64-snapshot trajectory, so deduplicating saves ~30 s of total test time). 12 tests pass in ~20 s on CPU with parallel threads: (1) **CFD L=32**: compile (rank=2, $K_2=1024$, $d=1024$), generate cosine sim 0.12, Bayesian overlap $\in \{0.81, 0.11\}$ for 2/8 (other 6 hit rank-collapse on the sparse M/d=1/16 data), no explosion. (2) **Ising L=32**: compile (rank=2, $K_2=1024$, $d=1024$), generate is mildly anti-correlated (-0.09, a known limitation of the rank-2 path on sparse data), Bayesian overlap $\in \{0.78, 0.70\}$ for 2/8, no explosion. (3) **GW d=1024**: compile (rank=2, $K_2=1024$, $d=1024$), generate cosine sim 0.42 (4 queries) and 0.10 (4 queries), Bayesian overlap $\in [0.05, 0.77]$ for 4/8, no explosion. Workspace test count: 259 → 271 (+12). The first item from the QFM.tex post-rev 29 "next steps" list is closed.
  ...
  - [x] **P11.20** — `arctic_authority` threshold-signed collective authority (rev 28, DONE): new `australVM/arctic_authority/` crate (MIT, deps on `../dynamic-arctic`); `ArcticAuthEngine` verifies a `DelegationCertificate` via `arctic_core::verify` and authorizes by capability + expiry. Bridged into `australVM/safestos/cranelift` as a new optional `arctic-auth` feature (`src/arctic_auth.rs`, `ArcticVmEngine: AuthorizationEngine`). 10 tests total (9 + 1), clippy clean alone/default/combined.
  ...
  - [x] **P11.23** — `unfer_nixvm` GPU-shared Nix execution sandbox (rev 28, DONE): `../unfer/flake.nix` gained `packages.x86_64-linux.unfer-ffi` (built and verified — a real `libunfer_ffi.so`); new subdirectory `unfer/unfer_nixvm/` composes it with `../cloud-hypervisor-build`'s existing `configuration.nix` (taken as a raw path input, not edited) into `vm-perf-with-unfer`/`vm-sec-with-unfer` NixOS image outputs, `nix eval`-verified well-formed (ran out of disk on `nix build` in this sandboxed environment — not a defect). Also fixed a real `--shared-dir ../nix` → `/nix` path bug in `full-stack-vm-launch.sh`. Booting the VM, `australVM`/`qfm` packages, and the `cuda` feature variant are follow-on work — see "Next steps."
...
## Workstream E — Quantum Flow Matching (QFM) module (QFMplan.md / QFM.tex, 2026-06-27)

> Adapted from `QFMplan.md` + `QFM.tex`: an analytical, neural-network-free
...
> three QFMplan stages done and verified end-to-end.
...
> The algorithm in `QFM.tex §8` is now code-complete. The new
...
- The full v1 system (rev 14/15) plus the v2 Bayesian update (rev
  ...
  `QFM.tex` algorithm, with one remaining v2 frontier item: P6 G
...
- **New `prob_kernel::Session::bayesian_update` method.** The
  QFM.tex §8 algorithm is now exposed as a session-level op. Takes
...
- Before rev 17, the `QFM.tex` algorithm had **two** open v2 frontier
...
> §4); the QFM Fokker–Planck transport of QFM.tex §2.3 therefore stays a
...
> implements all 5 phases of the `QFM.tex §8` algorithm. Rev 17
