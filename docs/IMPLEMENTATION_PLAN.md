...
## Next steps to improve (post-rev 18)

> **The v1 + v2 algorithm is closed and the closed algorithm is now productionized** (rev 18, 10 of 18 P7–P10 items done). The remaining 8 items are **research-grade (P8.7/P8.8/P8.9/P10.16–P18), cross-repo (P9.14/P9.15), or single-day follow-ups (P9.15)**. The next rounds of work are no longer "implement the algorithm" nor "productionize" but **scale, validate empirically, and complete the user-facing surface**.
>
...
>
> **P8.7 (Typst-math compiler), P8.8 (belief propagation), P8.9 (bayes_update_module v2), P9.14 (mathed_mini Step 4), P9.15 (stale velyst examples), P10.17 (Lindbladian), and P10.18 (Millennium-Prize demo)** are the other documented next steps. Each is a research-grade project of its own and will be tracked as its own revision when picked up.
...
9. **`bayes_update_module/` v2** (~1 week, **research**). The current `bayes_update_module/` Austral demo hardcodes a single observation at training point 0 with the default HMC options. A v2 demo could (a) drive multiple observations in a loop (consume the JSON result, feed it back as a new observation), (b) demonstrate a posterior-mean estimate (sample N points, compute the Karcher mean on the unit sphere of $\Cset^m$), and (c) expose a `generate_from_observations` op that returns the full `BayesianUpdateResult` (not just the HMC sample).
