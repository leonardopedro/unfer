...
**The remaining v2 frontier items** (P10 P16 real-dataset scaling, P10 P18 Millennium-Prize mass-gap demo) are the documented next steps; each is a research-grade project of its own and will be tracked as its own revision when picked up. (P8 P7 Typst-math compiler, P8 P8 belief propagation, P8 P9 `bayes_update_module` v2 — the Karcher-mean posterior estimate —, P9 P14 `mathed_mini` Step 4, and P9 P15 + P9.15.1 stale `velyst` examples port are all DONE in rev 19/20/21/22/23. P10 P17 Lindbladian dynamics is excluded by user decision 2026-06-30.)

## Next steps to improve (post-rev 23)

> **The v1 + v2 algorithm is closed, the closed algorithm is now productionized, the velysterm upstream typst 0.15 / KanvaSink / kanva_typst update is merged, the mathed_mini Step 4 (caret blink + a11y + click-to-place-caret + kernel_client wiring) is formalized with tests, and the 3 deleted velyst examples are now re-introduced and ported to the velyst 0.15 API** (rev 18–23, 16 of 18 P7–P10 items done: P7 all 6, P8.7, P8.8, P8.9, P8.10, P8.11, P9.12, P9.13, P9.14, P9.15, P9.15.1). The remaining items are **research-grade (P10.16 / P10.18)**. The next rounds of work are no longer "implement the algorithm" nor "productionize" but **scale, validate empirically, and complete the user-facing surface**.
>
> **The recommended next revision is P10.16 (real-data scaling, ongoing research)** — extending the MNIST 8×8 baseline (P7.3) to CIFAR-10 32×32, ImageNet 64×64, and cross-domain datasets (CFD, Ising-model critical snapshots). This is the central scientific claim of the QFM-TSR pipeline and has no size cap.
>
> **P10.18 (Millennium-Prize mass-gap demo on l=3 4D SU(2) Yang-Mills)** is the other documented next step. It is a research-grade project of its own and will be tracked as its own revision when picked up. (P8.7 — Typst-math compiler, P8.8 — chain belief propagation, P9.14 — mathed_mini Step 4, and P9.15.1 — deleted velyst examples port — are DONE in rev 20/21/22/23.)
