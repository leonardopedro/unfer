...
│   ├── unfer_ffi/              # handle-based C ABI: uk_*/uz_* symbols (census: EXPECTED_SYMBOLS*.txt)
...
- `unfer_ffi` exposes a C ABI for in-process module calls (`uk_*`/`uz_*`;
  authoritative census in `EXPECTED_SYMBOLS*.txt`, generated by
  `scripts/gen_symbol_artifacts`).
...
1. Create `$ROOT/<name>/` with `module.toml` granting the `uk_*` symbols the
   module calls (see `EXPECTED_SYMBOLS.txt` for the full census).
