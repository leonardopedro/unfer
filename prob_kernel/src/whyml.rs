...
/// Number of proof obligations the emitted program declares: the two subset
/// lemmas (reflexive, transitive), the `mem` postcondition (Why3 1.8 rejects
/// the logical `Mem.mem` predicate in program code, so a program-level `mem`
/// function links the two), plus the two postconditions (`authorize` and the
/// gate entrypoint).
